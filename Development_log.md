...
안정화
1.	다크모드 사용은 어떻게? (changemode?)
2.	전자칠판 동기화, 코드창 기능은 어디로?
3.	저장 판넬, 이미지 크기가 너무 작음
4.	복습 버튼의 기능은?
5.	Client 재접속 시 코드뿐만 아니라 코딩 문자 정보 또한 자동변환
6.	영상전송 기능은 아직 미구현?
7.	설계 확인 기능 미구현? 
8.	제출 기능 활성화 버튼은 어디? 그리고 아래 케이스는 비정상적? (제출 기능) 8-1, 8-2 참조
9.	설계 확인 메시지 추가(클라에서 명확한 확인 어려움)
10.	추가된 점수 확인이 어려움
11.	시험의 경우에는 점수 확인 창 불용
12.	더 코알라 웹버젼 파이썬 입력창 에러?
 	
디자인 및 보완사항
1.	design_memo의 텍스트에서 1. 변수설정 에서 “:” 추가로넣기
A.	1. 변수 설정 :
2.	파이썬 컴파일 코드에 &&Pause를 &Pause로 변경(서버, 클라이언트 둘다)
A.	RunProcess("cmd /c %APPDATA%\\..\\Local\\Programs\\Python\\Python37-32\\python.exe \""+CoalaPath+"\\code_py.py\" &&echo. &pause", false, true);
3.	저장 판넬의 판넬 크기, 이미지 크기 키우기
4.	클라이언트 실행 시 로그인창 크기 자동조절 이슈 고치기
A.	해결, Form2의 line 194, 195 주석처리하기
5.	Client 재접속 시 코드뿐만 아니라 코딩 문자 정보 또한 자동변환
